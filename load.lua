...
if not getgenv().Owners then getgenv().Owners = {} end
if not getgenv().Configuration then getgenv().Configuration = {whitelist = {}, Msg = "Stand activated!"} end
...
local function processCommand(speaker, message)
	...
	elseif cmd == ".commands" then
		...
		return
	elseif cmd == ".quit" and not isOwner(speaker) then
		if not hasAdminPermissions(speaker) then
...
local function onPlayerChatted(player, message)
	if message:lower() == "pricing" then
		showPricing(player)
	elseif message:lower() == "freetrial" then
		if not isFreeTrial(player) then
			table.insert(getgenv().FreeTrial, player.Name)
			makeStandSpeak("Thanks for redeeming free trial! You have 5 minutes to use commands. Type .commands to see available commands")
			spawn(function() processFreeTrial(player) end)
		else
			makeStandSpeak("You already have an active free trial")
		end
	end
